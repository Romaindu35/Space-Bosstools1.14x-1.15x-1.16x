...
public class MoonDimension extends BossToolsModElements.ModElement {
	...
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("boss_tools:moon")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("boss_tools:moon"), dimension, null, false);
		}
		type = DimensionType.byName(new ResourceLocation("boss_tools:moon"));
	}

	@Override
	public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[]{ForgeRegistries.BIOMES.getValue(new ResourceLocation("boss_tools:moon_biom")),};
	}
			...
			if (!biomesPatched) {
				for (Biome biome : this.biomes) {
					biome.addCarver(GenerationStage.Carving.AIR, Biome.createCarver(new CaveWorldCarver(ProbabilityConfig::deserialize, 256) {
						{
							carvableBlocks = ImmutableSet.of(MoonStoneBlock.block.getDefaultState().getBlock(),
