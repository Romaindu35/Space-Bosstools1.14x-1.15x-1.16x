...
public class MercuryDimension extends BossToolsModElements.ModElement {
	@ObjectHolder("boss_tools:mercury")
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("mercury"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("boss_tools:mercury")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("boss_tools:mercury"), dimension, null, false);
		}
		type = DimensionType.byName(new ResourceLocation("boss_tools:mercury"));
	}

	@Override
	public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[]{ForgeRegistries.BIOMES.getValue(new ResourceLocation("boss_tools:mercurybiome")),};
	}
		...
		public ChunkProviderModded(IWorld world, BiomeProvider provider) {
			super(world, provider, new OverworldGenSettings() {
				public BlockState getDefaultBlock() {
					return MrrcuryCobblestoneBlock.block.getDefaultState();
				}
			...
			if (!biomesPatched) {
				for (Biome biome : this.biomes) {
					biome.addCarver(GenerationStage.Carving.AIR, Biome.createCarver(new CaveWorldCarver(ProbabilityConfig::deserialize, 256) {
						{
							carvableBlocks = ImmutableSet.of(MrrcuryCobblestoneBlock.block.getDefaultState().getBlock(),
