...
public class MarsDimension extends BossToolsModElements.ModElement {
	...
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("boss_tools:mars")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("boss_tools:mars"), dimension, null, false);
		}
		type = DimensionType.byName(new ResourceLocation("boss_tools:mars"));
	}

	@Override
	public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[]{ForgeRegistries.BIOMES.getValue(new ResourceLocation("boss_tools:mars_biom")),};
	}
		...
		public ChunkProviderModded(IWorld world, BiomeProvider provider) {
			super(world, provider, new OverworldGenSettings() {
				public BlockState getDefaultBlock() {
					return MarsStoneBlock.block.getDefaultState();
				}
			...
			if (!biomesPatched) {
				for (Biome biome : this.biomes) {
					biome.addCarver(GenerationStage.Carving.AIR, Biome.createCarver(new CaveWorldCarver(ProbabilityConfig::deserialize, 256) {
						{
							carvableBlocks = ImmutableSet.of(MarsStoneBlock.block.getDefaultState().getBlock(),
